...
class SFRequest:
    ...
    def _set_cargo_detail(self, picking, product):
        cargo_info = dict()
        count = product.quantity_svl * -1
        cargo_info['count'] = count
        cargo_info['unit'] = product.weight_uom_name
        cargo_info['amount'] = count * product.list_price
        cargo_info['currency'] = product.currency_id.name
        cargo_info['name'] = product.name
